...
function showFreedom(freedomUntil) {
  ...
  function updateTimer() {
    ...
    if (diff > 0) {
      ...
      const percentage = ((totalDuration - diff) / totalDuration) * 100;
      timerProgress.style.background = `conic-gradient(#4caf50 ${360 - (percentage * 3.6)}deg, #333 0deg)`;
      setTimeout(updateTimer, 1000);
    } else {
      timerDiv.textContent = '00:00:00';
      timerProgress.style.background = `conic-gradient(#333 360deg, #333 0deg)`;
...
document.addEventListener('DOMContentLoaded', () => {
  chrome.storage.local.get([FREEDOM_KEY, ASSIGNED_PROBLEM_KEY, SELECTED_DIFFICULTY_KEY, STREAK_KEY, TIMER_DURATION_KEY], (result) => {
    const now = Date.now();
    ...
    const streakData = result[STREAK_KEY];
    if (streakData) {
        document.getElementById('streak-counter').textContent = streakData.streak;
    }

    const savedDuration = result[TIMER_DURATION_KEY] || 180;
    document.getElementById('timer-duration-input').value = savedDuration;
  });
  ...
  newProblemBtn.addEventListener('click', () => {
    ...
    chrome.runtime.sendMessage({ type: 'generate_new_problem', difficulty: selectedDifficulty });
  });
  ...
  timerDurationInput.addEventListener('change', () => {
    ...
    if (!isNaN(newDuration) && newDuration > 0) {
      chrome.storage.local.set({ [TIMER_DURATION_KEY]: newDuration });
    }
  });
});
...
chrome.storage.onChanged.addListener((changes, namespace) => {
  if (namespace === 'local') {
    ...
    if (changes[STREAK_KEY]) {
        const newStreak = changes[STREAK_KEY].newValue;
        if (newStreak) {
            document.getElementById('streak-counter').textContent = newStreak.streak;
        }
    }
  }
});